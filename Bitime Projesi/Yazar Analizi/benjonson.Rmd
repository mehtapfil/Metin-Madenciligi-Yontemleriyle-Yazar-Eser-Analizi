...
benjonson_eserleri<- gutenberg_metadata %>%
...
```{r}
mystopwords<-tibble(word = c("ı", "thou", "sır", "jonson", "thou", "ı'll", "ın", "la", "thy", "thee", "tis", "ıt", "ay", "ıf", "ıs", "jonson's", "1", "shakespeare", "_enter", "ha", "i'faith", "2", "_aside", "_exit", "ben", "3", "4", "_re", "avoc", "uh", "ly", "_ıt", "_you", "te", "1609"))
...
frequency <- bind_rows(mutate(tidy_benjonson, author = "Ben Jonson"),
  ...
  gather(author, proportion, `William Shakespeare`)
...
ggplot(frequency, aes(x = proportion, y = `Ben Jonson`, 
                      color = abs(`Ben Jonson` - proportion))) +
...
```{r}
cor.test(data = frequency[frequency$author == "William Shakespeare",],
         ~ proportion + `Ben Jonson`, method = "spearman")
```
...
```{r}
epicoene <- tidy_benjonson %>% 
  filter(title == "Epicoene; Or, The Silent Woman")
epicoene
```
...
```{r}
bing_word_counts <- tidy_benjonson %>%
  inner_join(get_sentiments("bing")) %>%
...
bigrams_separated <- benjonson_bigrams %>%
...
```{r}
benjonson_section_words <- benjonson %>%
  filter(title == "Epicoene; Or, The Silent Woman") %>%
...
word_pairs <- benjonson_section_words %>%
...
```{r}
word_cors <- benjonson_section_words %>%
  group_by(word) %>%
  filter(n() >= 20) %>%
...
```{r}
word_cors %>%
  filter(item1 %in% c("marry", "cutbeard", "captain")) %>%
  group_by(item1) %>%
...
benjonson_td <- tidy(benjonson_dtm)
benjonson_td
```
...
```{r}
benjonson_sentiments <- benjonson_td %>%
  inner_join(get_sentiments("bing"), by = c(term = "word"))
benjonson_sentiments
```
...
by_chapter <- books %>%
  ...
  mutate(chapter = cumsum(str_detect(
    text, regex("^act ", ignore_case = TRUE)
  ))) %>%
   filter(chapter > 0) %>%
  ungroup() %>%
